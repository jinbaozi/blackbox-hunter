...
def base_registry(tmp: Path, *, with_system_install: bool = False, fallback: bool = False) -> Path:
    tools = []
    if with_system_install:
        tools.append({
            "name": "needtool",
            "binary_name": "needtool",
            "execution_model": "host_binary",
            "install_priority": ["dnf", "manual"],
            "system_packages": {"dnf": ["needtool"]},
            "install_cmds": {"manual": "manual needtool"},
...
def test_user_rejects_install_hard_blocks() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        registry = base_registry(tmp, with_system_install=True)
        scan = tmp / "scan"
...
exit 0
'''
        write(tmp / "fakebin" / "dnf", fake_dnf, 0o755)
        registry = base_registry(tmp, with_system_install=True)
        scan = tmp / "scan"
        rc, _ = run_pty([sys.executable, str(ROOT / "tools" / "preflight.py"), "--package-type", "rpm", "--registry", str(registry), "--scan-root", str(scan)], env=env, input_text="y\n")
        assert rc == 0
        data = json.loads((scan / "env_check.json").read_text(encoding="utf-8"))
        tool = data["tools"][0]
...
def test_user_approves_fallback() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        scan = tmp / "scan"
        rc, _ = run_pty([sys.executable, str(ROOT / "tools" / "preflight.py"), "--package-type", "deb", "--registry", str(registry), "--scan-root", str(scan)], env=env, input_text="y\n")
        assert rc == 0
        data = json.loads((scan / "env_check.json").read_text(encoding="utf-8"))
